...
from connectomics.data.augmentation.transforms import RandCopyPasted, RandMixupd
...
class TestModelBlock(unittest.TestCase):
    ...
    def test_copypaste(self):
        ...
        np.random.seed(42)
        cp_augmentor = RandCopyPasted(
            keys=["image"], label_key="label", prob=1.0, max_obj_ratio=0.5
        )
