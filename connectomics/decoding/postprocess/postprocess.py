...
def watershed_split(
    ...
    """
    if vol.ndim != 3:
        raise ValueError(f"Expected 3D label array, got {vol.ndim}D with shape {vol.shape}")
...
def intersection_over_union(masks_true: np.ndarray, masks_pred: np.ndarray) -> np.ndarray:
    ...
    """
    overlap = compute_label_overlap(masks_true, masks_pred)

    n_pixels_pred = np.sum(overlap, axis=0, keepdims=True)
    n_pixels_true = np.sum(overlap, axis=1, keepdims=True)

    denom = n_pixels_pred + n_pixels_true - overlap
