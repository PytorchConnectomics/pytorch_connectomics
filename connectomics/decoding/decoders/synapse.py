...
def polarity2instance(
    ...
    if exclusive:
        ...
        # Non-exclusive mode expects uint8 input (0-255 range)
        if volume.max() <= 1.0 and np.issubdtype(volume.dtype, np.floating):
            temp = volume > thres  # float input in [0, 1]
        else:
            thres = int(255.0 * thres)
            temp = volume > thres  # uint8 input in [0, 255]
    ...
    if semantic:
        ...
        logger.info("Found %d pre- and %d post-synaptic segments.", num_pre, num_post)
        logger.info("There are %d synapses under a conservative estimate.", num_syn)
