...
def load_and_apply_best_params(cfg, checkpoint_path=None):
    ...
    and updates the merged runtime decoding config with optimized parameters.
    ...
    Example:
        ...
        >>> # cfg.decoding now has optimized decoding parameters
    """
    output_pred_dir = getattr(cfg.inference.save_prediction, "output_path", None)
    if not output_pred_dir:
        raise ValueError("Missing inference.save_prediction.output_path in configuration")
    ...
    # Apply to merged runtime decoding config
    if getattr(cfg, "decoding", None) is None:
        cfg.decoding = []

    # Find the decoder entry in the merged runtime decoding config
